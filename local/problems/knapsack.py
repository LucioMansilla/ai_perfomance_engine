...
class RemoveFromKnapSack(Action):
    def __init__(self, item: int):
        self.item = item

    def __repr__(self):
        return f"Remove {self.item} from knapsack"

    def is_enabled(self, state: KnapsackState) -> bool:
        return state.data[self.item] == 1

    def execute(self, state: KnapsackState) -> KnapsackState:
        new_state = state.data.copy()
        new_state[self.item] = 0
        return KnapsackState(new_state, state.weights, state.capacity, state.values)

    def __eq__(self, other) -> bool:
        return isinstance(other, RemoveFromKnapSack) and self.item == other.item


class KnapSackProblem(Problem):
    def __init__(self, initial: KnapsackState):
        super().__init__([PutInKnapSack(i) for i in range(len(initial.data))]
                         +[RemoveFromKnapSack(i) for i in range(len(initial.data))], initial)
